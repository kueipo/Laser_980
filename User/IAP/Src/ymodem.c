...
#else
	status = Data_Receive(&char1, 1, timeout);
#endif
	if (status == HAL_OK)
	{
		switch (char1)
...
#else
			if ((Data_Receive(&char1, 1, timeout) == HAL_OK) && (char1 == CA))
#endif
			{
				packet_size = 2;
			}
...
#else
			status = Data_Receive(&p_data[PACKET_NUMBER_INDEX], packet_size + PACKET_OVERHEAD_SIZE, timeout);
#endif
		...
		while ((file_done == 0) && (result == COM_OK))
		{
			...
			case HAL_OK:
							...
							if (aPacketData[PACKET_DATA_INDEX] != 0)
							{
								...
								*p_size = filesize;
								...
								if (filesize > opt_area_size)
								{
									/* End session */
									tmp = CA;
									Data_Transmit(&tmp, 1, NAK_TIMEOUT);
									Data_Transmit(&tmp, 1, NAK_TIMEOUT);
									result = COM_LIMIT;
								}
								...
								res = Erase_Flash(erase_addr, erase_size);
								if (res >= 0)
									erase_addr += res;

								if (Write_Flash(flashdestination, (uint8_t *)file_size, 4) >= 0)
								{
									flashdestination += 4;
								}
								else
              ...
              if (FLASH_If_Write(flashdestination, (uint32_t*) ramsource, packet_length/4) == FLASHIF_OK)
							...
							if (Write_Flash(flashdestination, (uint8_t *)ramsource, packet_length) >= 0)
#endif
							{
...
COM_StatusTypeDef Ymodem_Transmit(uint8_t *p_buf, const uint8_t *p_file_name, uint32_t file_size)
{
	...
	{
		/* Send Packet */
		Data_Transmit(&aPacketData[PACKET_START_INDEX], PACKET_SIZE + PACKET_HEADER_SIZE, NAK_TIMEOUT);
		...
		/* Wait for Ack and 'C' */
		if (Data_Receive(&a_rx_ctrl[0], 2, NAK_TIMEOUT) == HAL_OK)
		{
	...
	while ((size) && (result == COM_OK))
	{
		...
		while ((!ack_recpt) && (result == COM_OK))
		{
			...
			Data_Transmit(&aPacketData[PACKET_START_INDEX], pkt_size + PACKET_HEADER_SIZE, NAK_TIMEOUT);
			/* Send CRC or Check Sum based on CRC16_F */
#ifdef CRC16_F
			...
			/* Wait for Ack */
			if ((Data_Receive(&a_rx_ctrl[0], 1, NAK_TIMEOUT) == HAL_OK) && (a_rx_ctrl[0] == ACK))
			{
				ack_recpt = 1;
				if (size > pkt_size)
				{
	...
	while ((!ack_recpt) && (result == COM_OK))
	{
		...
		/* Wait for Ack */
		if (Data_Receive(&a_rx_ctrl[0], 1, NAK_TIMEOUT) == HAL_OK)
		{
			if (a_rx_ctrl[0] == ACK)
			{
				ack_recpt = 1;
			...
			{
				if ((Data_Receive(&a_rx_ctrl[0], 1, NAK_TIMEOUT) == HAL_OK) && (a_rx_ctrl[0] == CA))
				{
					HAL_Delay(2);
					result = COM_ABORT;
				}
