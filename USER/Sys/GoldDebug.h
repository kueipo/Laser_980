...
#define GOL_LOG_INFO		0
	...
	#endif
	#define Gol_log_d(...)                                    \
		GOL_LOG_PRINTF("[D] (%s:%d) ", __FUNCTION__, __LINE__); \
		GOL_LOG_PRINTF(__VA_ARGS__);                            \
		GOL_LOG_PRINTF("\n")
#else
	#ifdef Gol_assert
		#undef Gol_assert
	...
	#endif
	#define Gol_log_err(...)                                             \
		GOL_LOG_PRINTF("\033[31;22m[E] (%s:%d) ", __FUNCTION__, __LINE__); \
		GOL_LOG_PRINTF(__VA_ARGS__);                                       \
		GOL_LOG_PRINTF("\033[0m\n")
#else
	/* error level log */
	#ifdef Gol_log_err
		#undef Gol_log_err
