...
#define Console_log_d(...)                                               \
	CONSOLE_LOG_PRINTF("[D][CONSOLE] (%s:%d) ", __FUNCTION__, __LINE__); \
	CONSOLE_LOG_PRINTF(__VA_ARGS__);                                     \
	CONSOLE_LOG_PRINTF("\n")
#else
#ifdef Console_Assert
#undef Console_Assert
...
#define Console_log_err(...)                                                        \
	CONSOLE_LOG_PRINTF("\033[31;22m[E][CONSOLE] (%s:%d) ", __FUNCTION__, __LINE__); \
	CONSOLE_LOG_PRINTF(__VA_ARGS__);                                                \
	CONSOLE_LOG_PRINTF("\033[0m\n")
#else
/* error level log */
#ifdef Console_log_err
#undef Console_log_err
...
#define Console_log_info(...)                       \
	CONSOLE_LOG_PRINTF("\033[32;22m[I][CONSOLE] "); \
	CONSOLE_LOG_PRINTF(__VA_ARGS__);                \
	CONSOLE_LOG_PRINTF("\033[0m\n")
#else
/* info level log */
