...
static int check_and_update_part_cache(const struct fal_partition *table, size_t len)
{
    ...
    {
        fal_log_err("Initialize failed! No memory for partition table cache");
        return -2;
    }
#endif
    ...
    for (i = 0; i < len; i++)
    {
        ...
        if (flash_dev == NULL)
        {
            fal_log_d("Warning: Do NOT found the flash device(%s).", table[i].flash_name);
            continue;
        }

        if (table[i].offset >= (long)flash_dev->len)
        {
            fal_log_err("Initialize failed! Partition(%s) offset address(%ld) out of flash bound(<%d).",
...
int fal_partition_init(void)
{
    ...
    if (flash_dev == NULL)
    {
        fal_log_err("Initialize failed! Flash device (%s) NOT found.", FAL_PART_TABLE_FLASH_DEV_NAME);
        goto _exit;
    }
    ...
    {
        fal_log_err("Setting partition table end offset address(%ld) out of flash bound(<%d).", part_table_offset, flash_dev->len);
        goto _exit;
    }
    ...
    if (new_part == NULL)
    {
        fal_log_err("Initialize failed! No memory for table buffer.");
        goto _exit;
    }
    ...
    while (part_table_find_ok)
    {
        ...
        if (flash_dev->ops.read(part_table_offset - table_item_size * (table_num), (uint8_t *) new_part,
                table_item_size) < 0)
        {
            fal_log_err("Initialize failed! Flash device (%s) read error!", flash_dev->name);
            table_num = 0;
            break;
        }
        ...
        {
            fal_log_err("Initialize failed! No memory for partition table");
            table_num = 0;
            break;
        }
    ...
    if (table_num == 0)
    {
        fal_log_err("Partition table NOT found on flash: %s (len: %d) from offset: 0x%08x.", FAL_PART_TABLE_FLASH_DEV_NAME,
...
{
	const struct fal_flash_dev *flash_dev = NULL;
    fal_assert(part >= partition_table);
    fal_assert(part <= &partition_table[partition_table_len - 1]);
	
	flash_dev = part_flash_cache[part - partition_table].flash_dev;
	return flash_dev;
}
...
int fal_partition_read(const struct fal_partition *part, uint32_t addr, uint8_t *buf, size_t size)
{
    ...
    {
        fal_log_err("Partition read error! Partition address out of bound.");
        return -1;
    }
    ...
    if (flash_dev == NULL)
    {
        fal_log_err("Partition read error! Don't found flash device(%s) of the partition(%s).", part->flash_name, part->name);
        return -1;
    }
    ...
    if (ret < 0)
    {
        fal_log_err("Partition read error! Flash device(%s) read error!", part->flash_name);
    }
...
int fal_partition_write(const struct fal_partition *part, uint32_t addr, const uint8_t *buf, size_t size)
{
    ...
    if (addr + size > part->len)
    {
        fal_log_err("Partition write error! Partition address out of bound.");
        return -1;
    }
    ...
    if (flash_dev == NULL)
    {
        fal_log_err("Partition write error!  Don't found flash device(%s) of the partition(%s).", part->flash_name, part->name);
        return -1;
    }
    ...
    if (ret < 0)
    {
        fal_log_err("Partition write error! Flash device(%s) write error!", part->flash_name);
    }
...
int fal_partition_erase(const struct fal_partition *part, uint32_t addr, size_t size)
{
    ...
    if (addr + size > part->len)
    {
        fal_log_err("Partition erase error! Partition address out of bound.");
        return -1;
    }
    ...
    if (flash_dev == NULL)
    {
        fal_log_err("Partition erase error! Don't found flash device(%s) of the partition(%s).", part->flash_name, part->name);
        return -1;
    }
    ...
    if (ret < 0)
    {
        fal_log_err("Partition erase error! Flash device(%s) erase error!", part->flash_name);
    }
