...
#define RX_LENGTH_1				128
#define TX_LENGTH_1				128
...
{
	for (uint8_t index = 0; index < UART_ID_MAX; index++)
...
void BSP_Uart_Enable(uint8_t ucIndex)
{
	...
	huart = g_stUartCfg[ucIndex].huart;
 ...
 * @retval None.
 */
void BSP_Uart_EnableDMA(uint8_t UartID)
{
	UART_HandleTypeDef  *huart;
	uint8_t *buf;
	uint8_t size;
	
	huart = g_stUartCfg[UartID].huart;
	buf = g_stUartCfg[UartID].Rx_buffer;
	size = g_stUartCfg[UartID].Rx_size;

#if 1
	IRQn_Type DMAIRQn;
	DMAIRQn = g_stUartCfg[UartID].DMAIRQn;
...
{
	UART_HandleTypeDef *huart = g_stUartCfg[UartID].huart;
	
	if (UartID < UART_ID_MAX)
		RingBufferPut(g_stUartCfg[UartID].Tx_ring_buffer_p, p, size);
...
void BSP_UART_TxStart(UART_HandleTypeDef *huart)
{
	...
	if (huart->gState == HAL_UART_STATE_READY)
	{
		...
		if (index < UART_ID_MAX)
		{
			...
			#if 0	
				HAL_UART_Transmit_DMA(huart, g_stUartCfg[index].Tx_buffer, length);
			#else
				HAL_UART_Transmit_IT(huart, g_stUartCfg[index].Tx_buffer, length);
			#endif
			}
		}
	}
...
{
	uint8_t ch = (uint8_t)(huart->Instance->RDR & 0x00FF);
