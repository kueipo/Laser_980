...
#define YMODEM_TIME_MS(cnt)                    ((unsigned int)cnt/YMODEM_TIME_BASE)
...
typedef struct
{
    bool (*put_data)(unsigned char *data,unsigned int len);       /**< put data */
    bool (*get_data)(unsigned char *data,unsigned int len);       /**< put data */
} ymodem_port_t;

typedef struct
{
    unsigned char packet_data[PACKET_STX_PKG_SIZE]; /**< support 1K include 128 */
    unsigned int packet_data_cnt;                 /**< count about packet data cnt */
    unsigned int packet_data_size;                /**< count about packet data size */
    ymodem_state_t state;                   /**< ymodem state machine state */

    unsigned int recv_correct_cnt;                /**< when receive correct data ,this cnt +1 */
    unsigned int recv_error_cnt;                  /**< when receive error data ,this cnt +1 */

    unsigned int time_cnt;
    unsigned int nack_retry_cnt;
} ymodem_data_t;

typedef ymodem_code_t (*ymodem_cb_t)(uint8_t *buf, uint16_t len);
...
//
//*****************************************************************************
bool ymodem_init(ymodem_t *this);
...
ymodem_code_t ymodem_on_begin(uint8_t *buf, uint16_t len);
ymodem_code_t ymodem_on_data(uint8_t *buf, uint16_t len);
