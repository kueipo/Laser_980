...
{
	s_stLevelTcb.flag.bUpdata = true;
	s_stLevelTcb.flag.bUpdataBG = true;
	s_stLevelTcb.flag.bUpdataIcoUp = true;
	s_stLevelTcb.flag.bUpdataIcoDown = true;
	s_stLevelTcb.flag.bUpdataLevel = true;
}
...
{
	if (s_stLevelTcb.flag.bUpdata == false)
		return;
	...
	/*  */
	s_stLevelTcb.flag.bUpdata = false;
}
...
static void UI_Level_BG_Init(void)
{
	if (s_stLevelTcb.flag.bUpdataBG == false)
		return;

	char *img = IMG_BG;
	ImageDisplay(img, IMG_BG_X_START, IMG_BG_Y_START);

	s_stLevelTcb.flag.bUpdataBG = false;
}
...
//	s_stLevelTcb.flag.bUpdataIcoUp = false;
}
...
//		return;
//	
//	s_stLevelTcb.flag.bUpdataIcoDown = false;
}
...
static void UI_Level_Val_Level(void)
{
	if (s_stLevelTcb.flag.bUpdataLevel == false)
		return;
	...
	{
		lastlen = len;
		UI_ClearWindow(((LCD_WIDTH / 2) - (Font20.Width * 6) / 2), 110, ((LCD_WIDTH / 2) + (Font20.Width * 6) / 2), 110 + Font20.Height, WHITE);
	...
	UI_Paint_DrawNum(((LCD_WIDTH / 2) - (Font20.Width * lastlen) / 2), 110, number, &Font20, WHITE, LIGHTGREEN);

	s_stLevelTcb.flag.bUpdataLevel = false;
}
